...
public class SearchView extends FrameLayout implements View.OnClickListener {
	...
	private void initStyle(AttributeSet attrs, int defStyleAttr) {
		...
		if (attr != null) {
			...
			if (attr.hasValue(R.styleable.SearchView_search_version_margins)) {
				setVersionMargins(
						attr.getInt(R.styleable.SearchView_search_version_margins, VERSION_MARGINS_TOOLBAR_SMALL));
			}
			...
			if (attr.hasValue(R.styleable.SearchView_search_animation_duration)) {
				setAnimationDuration(
						attr.getInteger(R.styleable.SearchView_search_animation_duration, mAnimationDuration));
			}
			...
			if (attr.hasValue(R.styleable.SearchView_search_hide_on_keyboard_close)) {
				setShouldHideOnKeyboardClose(
						attr.getBoolean(R.styleable.SearchView_search_hide_on_keyboard_close, true));
			}
	...
	public void setHeight(float dp) {
		int height = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp,
	...
	public void setVersionMargins(int version) {
		...
		if (version == VERSION_MARGINS_TOOLBAR_SMALL) {
			...
			int leftRight = mContext.getResources()
					.getDimensionPixelSize(R.dimen.search_toolbar_margin_small_left_right);
			int bottom = 0;
	...
	// ---------------------------------------------------------------------------------------------
	/*
	 * private void setQuery2(CharSequence query) { mEditText.setText(query);
		...
		if (mVersion == VERSION_MENU_ITEM) {
			...
			if (animate) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
					...
					SearchAnimator.fadeOpen(mCardView, mAnimationDuration, mEditText, mShouldClearOnOpen,
	...
	public void close(boolean animate) {
		...
		if (mVersion == VERSION_MENU_ITEM) {
			if (animate) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
					SearchAnimator.revealClose(mCardView, mMenuItemCx, mAnimationDuration, mContext, mEditText,
							mShouldClearOnClose, this, mOnOpenCloseListener);
				} else {
					SearchAnimator.fadeClose(mCardView, mAnimationDuration, mEditText, mShouldClearOnClose, this,
							mOnOpenCloseListener);
				}
			} else {
	...
	private void reveal() {
		mCardView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				SearchAnimator.revealOpen(mCardView, mMenuItemCx, mAnimationDuration, mContext, mEditText,
